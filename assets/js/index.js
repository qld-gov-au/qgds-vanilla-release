...
{{/ifCond}}`;
  ...
  // src/components/callout/callout.hbs?raw
  var callout_default = `<!-- Callout -->
<section class="qld__body {{background}}" id="{{id}}">

  {{#ifCond type "==" "callout"}}

    {{#ifCond calloutType "==" "default"}}
      <div class="qld__callout">
        {{#if show_heading}}
          <h3 class="qld__callout__heading">{{heading}}</h3>
        {{else}}
          <h3 class="qld__callout__heading qld__callout__heading--sronly">{{heading}}</h3>
        {{/if}}
    ...
    {{#ifCond calloutType "==" "event"}}
      <div class="qld__callout qld__callout--calendar-event">
        {{#if show_heading}}
          <h3 class="qld__callout__heading">{{heading}}</h3>
        {{else}}
          <h3 class="qld__callout__heading qld__callout__heading--sronly">{{heading}}</h3>
        ...
        <time class="qld__callout--calendar-event__time" datetime="{{event.dateTime}}">
          {{#formatDate event.dateTime ""}}{{/formatDate}}
        </time>
  ...
  var callout_default2 = {
    init() {
    }
  };

  // src/components/callout/version.json
  var version_default4 = {
    component: {
      name: "Callout",
      description: "Callouts are sometimes referred to as 'pull quotes' or 'highlighted text.' In our Design System callouts can also be used as a visual clue to help direct a user's attention to important pieces of information. Our design system also uses a modified callout style for quotes and dates.",
      version: "1.0",
      status: "Draft",
      type: "design"
    }
  ...
  function Callout({ data, template = callout_default }) {
    callout_default2.init();
    const props = {
      data,
      template,
    ...
    return new Component("Callout", props);
  }
