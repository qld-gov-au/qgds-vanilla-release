...
{{/ifCond}}`;
  ...
  // src/components/call-to-action/call-to-action.hbs?raw
  var call_to_action_default = '{{! link v0.0.2}}\n<a href="{{url}}" {{#if id }} id="{{id}}" {{/if}} class="qld__cta-link{{#if view_all}} qld__cta-link--view-all{{/if}}">\n  {{{linktext}}}\n</a>';

  // src/components/call-to-action/call-to-action.js
  var call_to_action_default2 = {
    init() {
    }
  };

  // src/components/call-to-action/version.json
  ...
  // src/components/call-to-action/index.js
  function CallToAction({ data, template = call_to_action_default }) {
    call_to_action_default2.init();
    const props = {
      data,
      template,
      meta: version_default4 || {}
    };
    return new Component("CallToAction", props);
  }
